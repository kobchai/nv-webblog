        ...
        <form v-on:submit.prevent = "createBlog">
            ...
            <transition name="fade">
                <div class="thumbnail-pic" v-if="blog.thumbnail != 'null'">
                    <img style="margin-bottom:5px;" :src="BASE_URL+blog.thumbnail" alt="thumbnail">
                </div>
            </transition>

            <form enctype="multiple/form-data" novalidate>
                <div class="dropbox">
                    <input type="file" multiple :name="uploadFieldName" 
                        :disabled="isSaving" @change="filesChange($event.target.name, $event.target.files)
                        fileCount = $event.target.files.length" accept="image/*" class="input-file" />
                    <!-- <p v-if="isInitial || isSuccess"></p> -->
                    <p v-if="isInitial">
